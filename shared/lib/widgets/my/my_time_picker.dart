...
const Duration _kDialAnimateDuration = Duration(milliseconds: 350);
const double _kTwoPi = 2 * math.pi;
const Duration _kVibrateCommitDelay = Duration(milliseconds: 100);

enum _TimePickerMode { hour, minute }

const double _kTimePickerHeaderPortraitHeight = 112.0;
...
class _DayPeriodControl extends StatelessWidget {
  ...
  void _togglePeriod() {
    final int newHour = (fragmentContext.selectedTime.hour + TimeOfDay.hoursPerPeriod) %
          ...
          child: Padding(
            ...
            child: Align(
              ...
              child: Semantics(
                selected: !amSelected,
                child:
                    Text(materialLocalizations.postMeridiemAbbreviation, style: pmStyle),
              ),
            ),
          ),
        ),
...
class _TimePickerHeaderLayout extends MultiChildLayoutDelegate {
  ...
  void _layoutHorizontally(Size size, BoxConstraints constraints) {
    ...
    _positionPivoted(
        size.width, size.height / 2.0, childSizes, fragmentsFlattened, pivotIndex);
  }
  ...
  void _positionPivoted(double width, double y, Map<_TimePickerHeaderId, Size> childSizes,
    ...
    for (final fragment in fragments.skip(pivotIndex + 1)) {
  ...
  void _positionPiece(
    ...
    for (int i = fragments.length - 1; i >= 0; i -= 1) {
      ...
      x -= childSize.width;
      positionChild(
          fragment.layoutId, Offset(x, centeredAroundY - childSize.height / 2.0));
      x -= fragment.startMargin;
    }
  }
...
class _TimePickerHeader extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final TextTheme headerTextTheme = themeData.textTheme;
...
class _DialState extends State<_Dial> with SingleTickerProviderStateMixin {
  ...
  List<_TappableLabel> _build24HourInnerRing(TextTheme textTheme) {
    ...
    for (final timeOfDay in _amHours) {
  ...
  List<_TappableLabel> _build24HourOuterRing(TextTheme textTheme) {
    final List<_TappableLabel> labels = <_TappableLabel>[];
    for (final timeOfDay in _pmHours) {
      labels.add(_buildTappableLabel(
        textTheme,
        timeOfDay.hour,
        localizations.formatHour(timeOfDay,
  ...
  List<_TappableLabel> _buildMinutes(TextTheme textTheme) {
    ...
    final List<_TappableLabel> labels = <_TappableLabel>[];
    for (final timeOfDay in _minuteMarkerValues) {
      labels.add(_buildTappableLabel(
        textTheme,
        timeOfDay.minute,
...
/// [Navigator.pop].
class _MyTimePickerDialog extends StatefulWidget {
  /// Creates a material time picker.
  ///
  /// [initialTime] must not be null.
  const _MyTimePickerDialog({
    Key key,
...
class _MyTimePickerDialogState extends State<_MyTimePickerDialog> {
  @override
  void initState() {
    super.initState();
      ...
      child: OrientationBuilder(builder: (BuildContext context, Orientation orientation) {
        ...
        final Widget pickerAndActions = Container(
          ...
          child: Column(
            ...
            children: <Widget>[
              Expanded(
                  child: picker), // picker grows and shrinks with the available space
              actions,
            ],
          ),
        );
...
Future<TimeOfDay> showMyTimePicker({
  ...
  final Widget dialog = _MyTimePickerDialog(initialTime: initialTime);
